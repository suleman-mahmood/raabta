...
pub async fn get_user_internal_id(public_id: &str, pool: &PgPool) -> Result<i64, sqlx::Error> {
    let row = sqlx::query!(
        r#"
...
pub async fn get_class_internal_id(public_id: &str, pool: &PgPool) -> Result<i64, sqlx::Error> {
    let row = sqlx::query!(
        r#"
...
pub async fn get_chat_internal_id(public_id: &str, pool: &PgPool) -> Result<i64, sqlx::Error> {
    let row = sqlx::query!(
        r#"
